...
- Netlify hosts the static app and a small serverless intake boundary. The current boundary creates Kanbn tickets from a question and required reply email, supports a manually confirmed Get it done intake label, and accepts signed Kanbn webhooks; it does not persist Cases, payment events, or webhook events locally.
...
- The public UI uses a Netlify serverless boundary for Kanbn intake; no Case database, identity, attachment storage, payment verification, or automated webhook processing exists yet.
